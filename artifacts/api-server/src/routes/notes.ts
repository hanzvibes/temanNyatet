...
router.post('/notes', requireAuth, async (req, res) => {
  try {
    ...
    const row = await createRow(SHEET, req.userId!, { title: title ?? null, content: content ?? '', tags: tags ?? [] });
...
router.put('/notes/:id', requireAuth, async (req, res) => {
  try {
    ...
    const row = await updateRow(SHEET, req.params.id as string, req.userId!, updates);
    if (!row) {
